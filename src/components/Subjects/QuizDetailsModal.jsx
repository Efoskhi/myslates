    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-end">
      ...
      <div className="bg-white p-6 rounded-lg shadow-lg lg:w-1/3 w-5/6 h-full justify-end overflow-y-auto overscroll-y-auto">
        ...
        <div className=" text-xs">
          ...
          {/* Quiz Description */}
          <div className="mb-4">
            <label className="block font-semibold">Question Number</label>
            <input
              placeholder="Enter the question number (e.g. 3)"
              className="w-full mt-1 p-2 border rounded-md"
              value={inputs.question_number}
              onChange={(e) => handleInputs("question_number", e.target.value)}
            />
          </div>
            ...
            {["a", "b", "c", "d"].map((opt) => (
              <div key={opt} className="flex items-center gap-3">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleInputs(`option${opt.toUpperCase()}`, e.target.value)
                  }
                />
              </div>
            ))}
