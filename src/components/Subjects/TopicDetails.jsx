...
const TopicDetails = ({ section, handleCloseModal, callback }) => {
  ...
  const defaultWeekID = section === "Update" ? JSON.parse(inputs.week)?.id : "";

  return (
    <div className="lg:w-[50%] w-full mx-auto mt-10 p-6 bg-white shadow-md rounded-xl border border-blue-100">
        ...
        >
          ...
          {weeks.map((week, key) => (
            <option
              ...
              selected={week.id === defaultWeekID}
            >
              {week.title}
            </option>
          ))}
