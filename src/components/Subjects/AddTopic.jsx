...
const AddTopic = ({ hooks }) => {
  const { weeks, isLoading: isLoadingWeeks } = useWeeks({
    shouldGetWeeks: true,
  });
  const { terms, isLoading: isLoadingTerms } = useTerms({
    shouldGetTerms: true,
  });

  const { inputs, handleInput } = hooks;

  return (
    <div className="py-6 grid lg:grid-cols-2 grid-cols-1 lg:gap-6 gap-2 w-full">
      <div>
        <div className="grid lg:grid-cols-2 grid-cols-1 lg:gap-6 gap-2">
          <div>
            <label className="block text-gray-700 text-xs ">Week</label>
            <select
            ...
            >
              ...
              {weeks.map((week, key) => (
                <option option={JSON.stringify(week)} key={key}>
                  {week.title}
                </option>
              ))}
            </select>
          ...
          <div>
            <label className="block text-gray-700 text-xs">Term</label>
            <select
              className="w-full p-2 border rounded-lg"
            ...
            >
              ...
              {terms.map((terms, key) => (
                <option option={terms.title} key={key}>
                  {terms.title}
                </option>
              ))}
            </select>
        ...
        <div className="mt-4">
          <label className="block text-gray-700 text-xs">Topic Name</label>
          <input
            type="text"
            className="w-full p-2 border rounded-lg"
        ...
        <div className="mt-4">
          <label className="block text-gray-700 text-xs">Serial Number</label>
          <input
            type="text"
            className="w-full p-2 border rounded-lg"
